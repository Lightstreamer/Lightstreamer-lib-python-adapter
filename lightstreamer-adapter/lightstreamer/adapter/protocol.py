...
def read_token(tokens, index):
    try:
        ...
        raise RemotingException("Token not found") from ie
...
def toMap(tokens, start, length=None):
...
def seq(tokens, offset, length=None):
    stop = offset + length if length else None
    data = tokens[offset:stop]
    seq = [read(data, 'S', i) for i in range(0, len(data), 2)]
    return seq
